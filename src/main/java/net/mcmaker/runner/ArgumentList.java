...
public class ArgumentList {
		...
		for (int x = 0; x < args.length; x++) {
			if (!ended) {
				if ("--".equals(args[x])) { // '--' by itself means there are no more arguments
					...
					if (idx == -1 && x + 1 < args.length && !args[x + 1].startsWith("-")) { // Not in --key=value, so
																							// try and grab the next
																							// argument.
						ret.addArg(true, key, args[x + 1]); // Assume that if the next value is a "argument" then don't
															// use it as a value.
	...
	public String[] getArguments() {
		return entries.stream().flatMap(e -> Arrays.asList(e.get()).stream()).toArray(size -> new String[size]);
	}
